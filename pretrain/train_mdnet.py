...
def train_mdnet():
    ...
    with open(data_path, 'rb') as fp:
        data = pickle.load(fp, encoding='latin1') #VOT Dataset to train on
        #data = pickle._Unpickler(fp)
        #data = data.load()

    K = len(data)
    dataset = [None]*K
    for k, (seqname, seq) in enumerate(data.items()):#data.iteritems()):
        img_list = seq['images']
        gt = seq['gt']
    ...
    for i in range(opts['n_cycles']):
        ...
        k_list = np.random.permutation(K) #randomly select indices from the data
        prec = np.zeros(K)
        for j,k in enumerate(k_list): #j = index of k-value
            tic = time.time()
